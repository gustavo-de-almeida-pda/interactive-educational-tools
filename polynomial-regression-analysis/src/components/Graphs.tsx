      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        <div className="h-80 border border-slate-800 p-2 rounded bg-slate-50">
          ...
          <ResponsiveContainer width="100%" height="90%">
            <ComposedChart data={trainData} margin={{ top: 5, right: 5, bottom: 5, left: 5 }}>
              <CartesianGrid strokeDasharray="3 3" opacity={0.5} />
              <XAxis dataKey="x" type="number" domain={xDomain} allowDecimals={false} tickCount={11} />
        ...
        <div className="h-80 border border-slate-800 p-2 rounded bg-slate-50">
          ...
          <ResponsiveContainer width="100%" height="90%">
            <ComposedChart data={testData} margin={{ top: 5, right: 5, bottom: 5, left: 5 }}>
              <CartesianGrid strokeDasharray="3 3" opacity={0.5} />
              <XAxis dataKey="x" type="number" domain={xDomain} allowDecimals={false} tickCount={11} />
              <YAxis domain={[minY, maxY]} allowDecimals={false} />
              <Tooltip cursor={{ strokeDasharray: '3 3' }} />
            ...
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart data={resData} margin={{ top: 5, right: 5, bottom: 20, left: 20 }}>
                ...
                <YAxis domain={[-15, 15]} ticks={[-15, -12, -9, -6, -3, 0, 3, 6, 9, 12, 15]} allowDecimals={false} label={({ viewBox }: any) => {
                  ...
                  return (
                    <text x={cx} y={cy} transform={`rotate(-90, ${cx}, ${cy})`} textAnchor="middle" fill="#666" fontSize={12}>
                      <tspan x={cx} dy="-0.6em">Residuals</tspan>
                      <tspan x={cx} dy="1.2em">(y_te_noise_te - y_te_noise_hat_te)</tspan>
                    </text>
                  );
                }} />
                <Tooltip cursor={{ strokeDasharray: '3 3' }} />
